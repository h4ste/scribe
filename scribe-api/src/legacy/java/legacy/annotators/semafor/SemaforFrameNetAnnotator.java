package com.github.h4ste.scribe.legacy.annotators.semafor;
 ...
 * Time: 11:10 AM
 */
public class SemaforFrameNetAnnotator<D extends AbstractDocument> implements Annotator<D> {
  ...
  public <B extends D> void annotate(Document<B> document) {
    log.info("Processing {}", document.get(AbstractDocument.id));

    //initialize a temporary directory for semafor to read from and write to
    final File tempDir = Files.createTempDir();
    final String inFile = tempDir.getAbsolutePath() + document.get(AbstractDocument.id);
    ...
    try {
      ...
      if (!file.exists()) {
        log.warn("{} not found", document.get(AbstractDocument.id));
      }
      else {
