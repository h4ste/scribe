...
import com.github.h4ste.scribe.legacy.text.Text;
 ...
 */
public abstract class BratCorpus<D extends AbstractDocument> extends Corpus<D> {
  ...
  /**
   * This function should take a BratEntity and the containing document and annotate the correct scribe.Annotation
  ...
  /**
   * This function should take a BratRelation and the containing document and annotate the correct scribe.Relation
  ...
  protected void readAnnotations(final Document<D> document, File annotationFile) {
    try (BufferedReader reader = new BufferedReader(new FileReader(annotationFile))) {
      ...
      final Splitter splitter = Splitter.on(CharMatcher.WHITESPACE).omitEmptyStrings();
          log.warn("Empty entity found on doc {}", document.get(AbstractDocument.id));
      String line;
      while ((line = reader.readLine()) != null) {
        ...
        if (line.startsWith("T") ) {
          ...
          if (be.length() > 0) {
            ...
            log.warn("Empty entity found on doc {}", document.get(AbstractDocument.id));
//              throw new RuntimeException("Zero length entity on doc " + document.getAnnotations(AbstractDocument.id) + ": " + be.toString());
          }
        }
  ...
  protected void writeAnnotations(final Document<D> document, BratRules br){
    ...
    final File file = new File(getDir(document), document.get(AbstractDocument.id) + ".ann");
   ...
   */
  public static <D extends AbstractDocument> BratCorpus<D> tokenAndSentenceCorpus(final String location, final String tokenAnnSet, final String sentenceAnnSet) {
    return new BratCorpus<D>(new File(location)) {
      @Override
      protected void writeAnnotations(Document<D> document) {
        int eventCount = 0;
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(new File(getDir(document), document.get(AbstractDocument.id) + ".ann")))) {
  ...
  public Document<D> attachAnnotations(final Document<D> document) {
    readAnnotations(document, new File(getDir(document), document.get(AbstractDocument.id) + ".ann"));
    return document;
  }
  ...
  protected File getDir(final Document<?> document) {
    return getDir(document.get(AbstractDocument.id));
  }
  ...
  public void save(Document<D> document) {
    log.info("Writing {}", document.get(AbstractDocument.id));
    final File textFile = new File(getDir(document), document.get(AbstractDocument.id) + ".txt");
    if (!textFile.exists()) {
  ...
  protected static class BratEntity {
    ...
    private BratEntity(final Iterator<String> line) {
      try {
        ...
        System.out.printf("Cannot annotate entity from line: |%s|\n\n",
  ...
  public static class BratRules {
    ...
    public void load(String filename) throws Exception {
      ...
      while(file.hasNextLine()){
        String line = file.nextLine().trim();//getAnnotations the next line
    ...
    public <D extends AbstractDocument> void processCorpus(BratCorpus<D> bc, String as){
