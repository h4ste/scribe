...
import com.github.h4ste.scribe.legacy.text.HasAttributes;
...
import com.github.h4ste.scribe.legacy.util.TieredHashing;
 ...
 */
public class JsonCorpus<D extends AbstractDocument> extends Corpus<D> implements Serializable {
...
//  private final Progress
  ...
  public static <D extends AbstractDocument> Builder<D> builder(final String textPath) {
    return new Builder<>(textPath);
  }

  public static <D extends AbstractDocument> Builder<D> at(final File path) {
    return at(path.getAbsolutePath());
  }

  public static <D extends AbstractDocument> Builder<D> at(final Path path) {
    return at(path.toAbsolutePath().toString());
  }

  public static <D extends AbstractDocument> Builder<D> at(final String path) {
  ...
  public static class Builder<D extends AbstractDocument> {
   ...
   * @param id the AbstractDocument.id of the document to be read
   * @return a Document
   */
  @Override
   ...
   *
   * To serialize different annotation sets than were read in using this corpus, annotate a new corpus and use
  ...
  public void save(final Document<D> document) {
    ...
    String feat = featPath;
    String hashDirs = null;
    final String docId = document.get(AbstractDocument.id);
    if (tiered) {
  ...
  private  Document<D> readText(File text) {
    try (BufferedReader reader = new BufferedReader(new FileReader(text))) {
      ...
      document.set(AbstractDocument.id, id);
      document.set(AbstractDocument.path, text.getAbsolutePath());
      return document;
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  ...
  private void readAnnotations(final Document<D> doc, final String annSet, final String path) {
    final File file = new File(path + File.separator + doc.get(AbstractDocument.id) + ".json");
//    log.info("Reading annotations from {}", file.getPath());
    if (!file.exists() || file.length() == 0) {
      /* TODO: this doesn't work -- if you have multiple streams or processes sharing a json corpus the counts getAnnotations crazy
...
//      }
    ...
    while ((jt = jp.nextToken()) != JsonToken.END_OBJECT) {
      ...
      if ("governor-id".equals(currentFieldName)) {
        ...
        assert ur != null : String.format(
            "Cannot annotate unsafe relation. GovId = %d, type = %s, DepId = %d. Ids of 0 indicate they were unassigned.",
            govId, type, depId);
  ...
  private void readFeaturemap(final JsonParser jp, final HasAttributes mapHaver) throws IOException, ClassNotFoundException {
    JsonToken jt;
    while ((jt = jp.nextToken()) != JsonToken.END_OBJECT) {
      if (jt == JsonToken.START_OBJECT) {
...
//              log.info("Setting {} to {}/{}.", key, clazz, jp.getText());
  ...
  private void writeText(final Document<D> doc, final File path) {
    try {
      final File file = new File(path, doc.get(AbstractDocument.id) + ".txt" + compression.ext);
      if (!file.exists()) {
        ...
        log.trace("Text Document at  already exists, skipping...", file.getAbsolutePath(), doc.get(AbstractDocument.id));
      }
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  ...
  private void writeDocumentFeatures(final Document<D> doc, final File parent) {
    if (!doc.cleanFeats()) {
      try {
        ...
        final File file = new File(parent, doc.get(AbstractDocument.id) + ".json" + compression.ext);
  ...
  private void writeAnnotations(final Document<D> doc, final File parent, final String annSet) {
    ...
    if (!unsafeAnnotations.isEmpty()) {
      try {
        ...
        final File file = new File(parent, doc.get(AbstractDocument.id) + ".json");
        ...
        try (JsonGenerator jg = jf.createGenerator(out, JsonEncoding.UTF8).useDefaultPrettyPrinter()) {
          jg.writeStartObject();
          {
            ...
            {
              jg.writeArrayFieldStart("Record");
