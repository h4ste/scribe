 ...
 */
public abstract class AnaphoraCorpus<D extends AbstractDocument> extends Corpus<D> {
  ...
  public void save(Document<D> document) {
    final File textFile = getTextDir(document).resolve(document.get(AbstractDocument.id) + ".txt").toFile();
    if (!textFile.exists()) {
  ...
  protected Path getTextDir(final Document<D> document) {
    return getTextDir(document.get(AbstractDocument.id));
  }
  ...
  protected Path getAnnotationDir(final Document<D> document) {
    return getAnnotationDir(document.get(AbstractDocument.id));
  }
